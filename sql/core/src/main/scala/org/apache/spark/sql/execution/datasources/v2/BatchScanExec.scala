  ...
  override def outputPartitioning: Partitioning = {
    super.outputPartitioning match {
      case k: KeyGroupedPartitioning if spjParams.commonPartitionValues.isDefined =>
        ...
        val kexpr = k.expressions
        val newExpressions = if (group) {
          projectIfNecessary(k.expressions, spjParams.joinKeyPositions, "partition exprs")
        } else {
          k.expressions
        }

        val finalPartValues = if (group) {
      ...
      val finalPartitions = outputPartitioning match {
        case p: KeyGroupedPartitioning =>
          if (conf.v2BucketingPushPartValuesEnabled &&
            ...
            val finalPartitionMapping = if (group) {
            ...
            val expectedPartitions = if (group) p.uniquePartitionValues else p.partitionValues
...
//      expectedPartValues, partExpressions, spjParams.joinKeyPositions)
  ...
  private def groupPartitionsByJoinKey(
    ...
    if (partitionGroupByPositions.isDefined) {
      ...
      partitions.foreach { p =>
        groupedPartitions.add(
          partitionRow(p, partitionExpressions))
      }
  ...
  private def partitionRow(row: InternalRow,
    partitionExpressions: Seq[Expression]): InternalRowComparableWrapper = {
    partitionRow(row, partitionExpressions, spjParams.joinKeyPositions)
  }
  ...
  def group: Boolean = spjParams.joinKeyPositions.isDefined &&
      (!conf.v2BucketingPartiallyClusteredDistributionEnabled ||
