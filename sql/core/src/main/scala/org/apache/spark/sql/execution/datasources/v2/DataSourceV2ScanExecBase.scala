...
trait DataSourceV2ScanExecBase extends LeafExecNode {
  ...
  def groupPartitions(
    ...
    keyGroupedPartitioning.flatMap { expressions =>
      ...
      if (results.length != inputPartitions.length || inputPartitions.isEmpty) {
        ...
        val (finalPartitions, finalExpressions) = if (groupSplits) {
          ...
          val parts = results.map(t => (t._1, Seq(t._2)))
          (parts, expressions)
        }

        val partitionOrdering: Ordering[(InternalRow, Seq[InputPartition])] = {
          RowOrdering.createNaturalAscendingOrdering(finalExpressions.map(_.dataType)).on(_._1)
        }
        Some(finalPartitions.sorted(partitionOrdering))
      }
    }
  }
  ...
  def projectedRow(row: InternalRow,
    ...
    if (joinKeyPositions.isDefined) {
      val values = row.toSeq(projectedExpressions.map(_.dataType))
      val filteredValues = projectIfNecessary(values, joinKeyPositions,
        "partition values")
      InternalRow.fromSeq(filteredValues)
    } else {
      row
    }
  }
  ...
  def projectedComparator(row: InternalRow,
    ...
    if (joinKeyPositions.isDefined) {
      ...
      val filteredValues = projectIfNecessary(values, joinKeyPositions,
        "partition values")
      val projectedExpressions = projectIfNecessary(partitionExpressions, joinKeyPositions,
        "partition expressions")
      InternalRowComparableWrapper(InternalRow.fromSeq(filteredValues), projectedExpressions)
    } else {
