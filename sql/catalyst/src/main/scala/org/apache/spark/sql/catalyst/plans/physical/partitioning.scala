...
trait ShuffleSpec {
   ...
   * A None value in the set indicates that the particular partition expression is not reducible on the
   * corresponding expression on the other shuffle spec.
   * <p>
   * Returning none also indicates that none of the partition expressions can be reduced on the corresponding
   * expression on the other shuffle spec.
   */
